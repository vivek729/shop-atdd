# Stderr-swallow sweep — issue #74

**Source:** https://github.com/optivem/shop/issues/74 — "Stderr-swallow sweep 2026-06-08"
**Scope (chosen):** All four repos — `actions`, `gh-optivem`, `hub`, `shop`.
**Counts from the sweep:** NEW 1 · PERSISTING 58 · RESOLVED 0 (59 findings total).
**Skipped by the sweep:** `optivem/courses` (private/inaccessible); 6 `command -v <cli> &>/dev/null` existence probes (correct pattern, excluded).
...
2. **Resolve every finding** — apply capture-and-surface to all 59, including existence probes and
...
3. **Verify** per repo:
   ...
   - `shop`: changed `.github/actions/*.yml` are `run:` blocks — `bash -n` the extracted snippet;
