...
- **Q4 — cover .NET/TS now, or just note them? → (a) Java POC + written note.** ~~Prove the mechanism in Java only; survey the non-JVM equivalents in writing (Step 7).~~ **🔄 OVERRIDDEN 2026-06-20 (user): full parity now in scope.** After the Java POC landed, the user chose to make .NET and TS consistent — both get the driver refactor **and** the A1/A2/A7/A10 architecture-rule tests (ArchUnitNET for .NET, ts-arch/ts-morph for TS). This work lives in the C2/C3 refactor plans (`...-dotnet.md` D8, `...-typescript.md` T8), driven by the coordinator. Step 7's written survey stands as the design basis for those steps.
